...
devtools::install_git("gitlab@gitlab.utu.fi:pajosm/iloreg.git",
                      credentials = creds,ref = "85196be6")
```
...
in which *L* ICP solutions are computed.

In the following, we give a brief summary of the parameters.

* $k$: The number of initial clusters in ICP. Default $15$. A lower value generates less cluster probabilities. Along with decreasing $d$ , increasing $k$ is the easiest way to increase the complexity of the outcome, i.e. more subpopulations with subtle differences emerge.
...
* $d$: A real number greater than $0$ and smaller than $1$ that determines how many cells $n$ are down- or oversampled from each cluster into the training data. ($d$ in $n=Nd/k$), where $N$ is the total number of cells. Default $0.3$. Our recommendation is to decrease $d$ below $0.2$, as this leads to ICP becoming unstable. Increasing $d$, on the other hand, leads to more dissimilar ICP runs and can be used to decrease the heterogeneity of the result.
* $r$: A positive integer that denotes the maximum number of reiterations performed until the ICP algorithm stops. Default $5$.
...
* $threads$: The number of threads to use in parallel computing. Default is $0$ to use all available threads but one. The parallelization can be disabled by setting $threads=1$. 
...
# Draw violin plots of the projection accuracy of ICP, the number of epochs and the average pairwise ARI.
...
PCA transformation requires the user to select the number of principal components, for which we selectedt the default value $p=50$. The elbow plot is commonly used to seek a point of inflection, from which proximity the user selects $p$. In this case it would be close to $p=10$.
...
```{r, fig.height=6, fig.width=7.2, out.width = '100%'}
...
```{r FigTop1Scatter, echo=TRUE, fig.height=12, fig.width=16, out.width = '100%', eval = TRUE}
GeneScatterPlot(sce,
