...
```{r}
GeneScatterPlot(iloreg_object,c("CD3D","CD79A","CST3","FCGR3A"),dim.reduction.type = "umap")
GeneScatterPlot(iloreg_object,c("CD3D","CD79A","CST3","FCGR3A"),dim.reduction.type = "tsne")
...
gene_markers <- FindAllGeneMarkers(iloreg_object,
                                   ...
                                   min.cells.group = 3,
                                   return.thresh = 0.01,
                                   only.pos = TRUE,max.cells.per.cluster = NULL)
...
GeneScatterPlot(iloreg_object,genes = unique(top1_log2FC$gene),dim.reduction.type = "tsne",point.size = 0.7)
```


```{r FigTop1Scatter2, echo=TRUE, fig.height=16, fig.width=20}
GeneScatterPlot(iloreg_object,genes = unique(top1_log2FC$gene),dim.reduction.type = "umap",point.size = 0.7)
```
...
GeneDropoutRatePlot(iloreg_object,genes = c("S100A12","CD14"),return.plot = F,use.clusters = c(19,17,25,11,21,29,12))
```


### Find markers betweent two arbitrary sets of clusters

```{r}
gene_markers_ <- FindGeneMarkers(iloreg_object,c(30,22,93),c(1),clustering.type = "manual",only.pos = T)
```
