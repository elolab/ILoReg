...
```{r}
iloreg_object <- ManualClustering(iloreg_object,K=25)
```
...
```{r}
GeneScatterPlot(iloreg_object,"CCR7",dim.reduction.type = "umap")
GeneScatterPlot(iloreg_object,"CCR7",dim.reduction.type = "tsne")
```
...
```{r Fig0, echo=TRUE, fig.height=7, fig.width=14}
p1 <- ClusterScatterPlot(iloreg_object,"manual",dim.reduction.type = "umap",return.plot = T)+ggplot2::ggtitle("Before merging 23 and 6")
iloreg_object <- MergeClusters(iloreg_object,clusters.to.merge = c(23,6))
p2 <- ClusterScatterPlot(iloreg_object,"manual",dim.reduction.type = "umap",return.plot = T)+ggplot2::ggtitle("After merging 23 and 6")
...
Find gene markers for clusters F and A from the manual clustering
```{r}

gene_markers_F_A <- FindGeneMarkers(iloreg_object,clusters.1 = "F",clusters.2 = "A",
                                   clustering.type = "manual",
                                   test = "wilcox",
                                   logfc.threshold = 0.25,
                                   min.pct = 0.25,
...
cowplot::plot_grid(plotlist = lapply(unique(top10$gene),function(x) GeneScatterPlot(iloreg_object,x,dim.reduction.type = "tsne",return.plot = T)))
```



Make gene expression violin plots of the top 1 markers
```{r Fig3, echo=TRUE, fig.height=20, fig.width=40}
VlnPlot(iloreg_object,clustering.type = "manual",genes = unique(top1$gene))
```

Make gene marker heatmap for the manual clustering
```{r Fig4, echo=TRUE, fig.height=15, fig.width=15}
...
```{r}
pheatmap::pheatmap(iloreg_object@normalized.data[unique(top10$gene),],show_colnames = FALSE)
```
